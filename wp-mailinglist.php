...
if (!class_exists('wpMail')) {
	class wpMail extends wpMailPlugin {
		...
		function importusers_hook() {
			header("HTTP/1.1 200 OK");
			global $wpdb, $Db, $Mailinglist, $Subscriber, $Field;
		...
		function autoresponders_hook() {
			...
			if ($autoresponderemails = $wpdb -> get_results($query)) {					
		...
		function admin_menu() {
			...
			add_object_page(__('Newsletters', $this -> plugin_name), __('Newsletters', $this -> plugin_name) . $update_icon, 'newsletters_welcome', $this -> sections -> welcome, array($this, 'admin'), $this -> url() . '/images/icon.png');
		...
		function admin_head_settings_templates() {
			...
			add_meta_box('unsubscribediv', __('Unsubscribe Email', $this -> plugin_name) . $Html -> help(__('Email message sent to the administrator when a subscriber unsubscribes.', $this -> plugin_name)), array($Metabox, 'settings_templates_unsubscribe'), "newsletters_page_" . $this -> sections -> settings_templates, 'normal', 'core');
			...
			switch ($_GET['method']) {			
				...
				case 'history'		:
					if ($history = $History -> get($_GET['id'])) {	
						if (empty($_GET[$this -> pre . 'updated'])) {
							//$this -> render_message(__('History/draft email has been loaded into the subject field and editor below.', $this -> plugin_name));
						}
							...
							if (!empty($_FILES['attachments']['name'])) {
								...
								foreach ($_FILES['attachments']['name'] as $fkey => $attachmentfile) {
									if ($_FILES['attachments']['error'][$fkey] == 0) {
										...
										$attpath = $Html -> uploads_path() . DS;
										$attfull = $attpath . $attfile;
		...
		function admin_importexport() {
			global $wpdb, $Db, $Html, $Queue, $Field, $Subscriber, $SubscribersList, $Mailinglist;
							...
							if ($fh = fopen($fileFull, "r")) {
								...
								if (!empty($_POST['importlists'])) {								
									foreach ($_POST['importlists'] as $importlist_id) {
										if ($mailinglist = $wpdb -> get_row("SELECT `id`, `paid` FROM `" . $wpdb -> prefix . $Mailinglist -> table . "` WHERE `id` = '" . $importlist_id . "'")) {
													...
													foreach ($addlistsarr as $addlisttitle) {
														...
														if (!empty($mailinglist_id)) {
															if (empty($mailinglists) || (!empty($mailinglists) && !in_array($mailinglist_id, $mailinglists))) {
																...
																if ($mailinglist = $wpdb -> get_row("SELECT `id`, `paid` FROM `" . $wpdb -> prefix . $Mailinglist -> table . "` WHERE `id` = '" . $mailinglist_id . "'")) {
																	$paid = (empty($mailinglist -> paid) || $mailinglist -> paid == "N") ? "N" : "Y";
												...
												case 'other'				:											
													switch ($field -> type) {
														...
														case 'checkbox'				:
															...
															break;
														case 'pre_country'			:
															$country = $wpdb -> get_var("SELECT `value` FROM " . $wpdb -> prefix . $wpmlCountry -> table . " WHERE `id` = '" . $subscriber -> {$field -> slug} . "'");
					...
					if (!empty($_GET['id'])) {
						if ($email = $Db -> find(array('id' => $_GET['id']))) {											
							...
							if ($this -> execute_mail($subscriber, false, $email -> subject, $message, $email -> attachments, $email -> history_id, $eunique)) {
								...
								$msg_type = 'error';
								$message = __('Email could not be sent, please check email settings', $this -> plugin_name);
							}
						} else {
							$msg_type = 'error';
			...
			switch ($_GET['method']) {
				...
				case 'duplicate'		:
					if (!empty($_GET['id'])) {
						...
						$wpdb -> query($query);
						$query = "UPDATE `historytmp` SET `id` = '" . $nextid . "', `sent` = '0' WHERE `id` = '" . $_GET['id'] . "'";
						$wpdb -> query($query);
