	...
	<form action="?page=<?php echo $this -> sections -> themes; ?>&amp;method=mass" method="post" onsubmit="if (!confirm('<?php _e('Are you sure you wish to execute this action on the selected themes?', $this -> plugin_name); ?>')) { return false; }" id="themesform">
		<div class="tablenav">
		...
		<table class="widefat">
			...
			<tbody>
				<?php if (empty($themes)) : ?>
					<tr class="no-items">
						<td class="colspanchange" colspan="<?php echo $colspan; ?>"><?php _e('No themes were found', $this -> plugin_name); ?></td>
					</tr>
				<?php else : ?>
					...
					<?php $class = ($class == "alternate") ? '' : 'alternate'; ?>
						<tr class="<?php echo $class; ?>" id="templaterow<?php echo $theme -> id; ?>">
							...
							<td>
								...
								<?php echo (!empty($theme -> defsystem) && $theme -> defsystem == "Y") ? ' <small>(' . __('System Default', $this -> plugin_name) . ' <a onclick="if (!confirm(\'' . __('Are you sure you want to remove this theme as the system default?', $this -> plugin_name) . '\')) { return false; }" class="newsletters-icon-delete-regular" href="' . admin_url('admin.php?page=' . $this -> sections -> themes . '&method=remove_defaultsystem&id=' . $theme -> id) . '"></a>)</small> ' . $Html -> help(__('This theme is used for system emails such as confirmation, unsubscribe, authentication and other system notifications.', $this -> plugin_name)) : ''; ?>
								<?php echo (!empty($theme -> def) && $theme -> def == "Y") ? ' <small>(' . __('Send Default', $this -> plugin_name) . ' <a onclick="if (!confirm(\'' . __('Are you sure you want to remove this theme as the sending default?', $this -> plugin_name) . '\')) { return false; }" class="newsletters-icon-delete-regular" href="' . admin_url('admin.php?page=' . $this -> sections -> themes . '&method=remove_default&id=' . $theme -> id) . '"></a>)</small> ' . $Html -> help(__('This theme is used for sending and will be pre-selected under Newsletters > Create Newsletter for example.', $this -> plugin_name)) : ''; ?>
								<div class="row-actions">
									...
									<span class="delete"><?php echo $Html -> link(__('Delete', $this -> plugin_name), '?page=' . $this -> sections -> themes . '&amp;method=delete&amp;id=' . $theme -> id, array('onclick' => "if (!confirm('" . __('Are you sure you want to delete this theme?', $this -> plugin_name) . "')) { return false; }", 'class' => "submitdelete")); ?> |</span>
									<span class="view"><?php echo $Html -> link(__('Preview', $this -> plugin_name), "", array('onclick' => "jQuery.colorbox({href:'" . home_url() . '/?' . $this -> pre . 'method=themepreview&id=' . $theme -> id . "'}); return false;", 'title' => __('Theme Preview: ', $this -> plugin_name) . $theme -> title)); ?></span>
								</div>
							</td>
							<td><label for="checklist<?php echo $theme -> id; ?>"><abbr title="<?php echo $theme -> modified; ?>"><?php echo date("Y-m-d", strtotime($theme -> modified)); ?></abbr></label></td>
						</tr>
					<?php endforeach; ?>
				<?php endif; ?>
			</tbody>
		...
		<div class="tablenav">
			<div class="alignleft">
				...
				function change_sorting(field, dir) {
					document.cookie = "<?php echo $this -> pre; ?>themessorting=" + field + "; expires=<?php echo date_i18n($this -> get_option('cookieformat'), strtotime("+30 days")); ?> UTC; path=/";
					document.cookie = "<?php echo $this -> pre; ?>themes" + field + "dir=" + dir + "; expires=<?php echo date_i18n($this -> get_option('cookieformat'), strtotime("+30 days")); ?> UTC; path=/";
					window.location = "<?php echo preg_replace("/\&?" . $this -> pre . "page\=(.*)?/si", "", $_SERVER['REQUEST_URI']); ?>";
				}
				</script>
			</div>
